...
import type { TableRow } from "~/@types/mdast";
import { chooseRegex } from "@/services/prosemirror/tokens/html-tokens";
import type { Value as UnifiedValue } from "~/unified/lib";

type CustomRemarkProsemirrorOptions = Omit<RemarkProseMirrorOptions, 'handlers'> & {
  handlers: RemarkProseMirrorOptions['handlers']
}

export type Value = Parameters<typeof markdownToProseMirror>["0"];
...
export async function markdownToProseMirror(markdown: UnifiedValue): Promise<Node> {
  const doc = await unified()
    ...
    } satisfies CustomRemarkProsemirrorOptions)
    .process(markdown);
